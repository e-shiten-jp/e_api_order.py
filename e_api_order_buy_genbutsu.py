...
# p4-5/46 No.5 CLMKabuNewOrder 項目1-28 を参照してください。
#
#
#5       CLMKabuNewOrder             I/O -
...
#    9       sTategyokuZyoutoekiKazeiC   建玉譲渡益課税区分  char[1] I   信用建玉における譲渡益課税区分（現引、現渡で使用） *：現引、現渡以外の取引 1：特定 3：一般 5：NISA
...
#    14      sOrderPrice        注文値段        char[14]    I   *：指定なし 0：成行 上記以外は、注文値段 小数点については、関連資料：「立花証券・ｅ支店・ＡＰＩ、REQUEST I/F、マスタ データ利用方法」の「２－１２． 呼値」参照
#    15      sOrderSuryou       注文数量        char[13]    I   注文数量
#    16      sGenkinShinyouKubun 現金信用区分   char[1]     I   0：現物 2：新規(制度信用6ヶ月) 4：返済(制度信用6ヶ月) 6：新規(一般信用6ヶ月) 8：返済(一般信用6ヶ月)
#    17      sOrderExpireDay    注文期日        char[8]     I   0：当日 上記以外は、注文期日日(YYYYMMDD)[10営業日迄]
#    18      sGyakusasiOrderType 逆指値注文種別 char[1]     I   0：通常 1：逆指値 2：通常＋逆指値
#    19      sGyakusasiZyouken  逆指値条件      char[14]    I   0：指定なし 条件値段（トリガー価格）
#    20      sGyakusasiPrice    逆指値値段      char[14]    I   *：指定なし 0：成行 上記以外は逆指値値段
#    21      sTatebiType        建日種類        char[1]     I   *：指定なし（現物または新規） 1：個別指定 2：建日順 3：単価益順 4：単価損順
#    22      sSecondPassword    第二パスワード  char[48]    I   第二暗証番号 ##：第二暗証番号省略時 関連資料：「立花証券・ｅ支店・ＡＰＩ、インターフェース概要」の「３－２．ログイン、ログアウト」参照
...
#    26      sKinri             金利            char[9]     O   メモリ上のシステム市場弁済別取扱条件信用新規取引の場合 0～999.99999：買方金利 0～999.99999：売方金利 0～999.99999：買方金利（翌営業日） 0～999.99999：売方金利（翌営業日） -：信用新規取引でない場合
